...
if (isset($_POST['sub'])) {
    ...
    if (!in_array($file_extension, $allowed_extensions)) {
        ...
        if (move_uploaded_file($tem_img, $upload_dir . $img)) {
            $q = "INSERT INTO home_slider(main_text, p_text, IMAGE) VALUES ('$p_name', '$description', '$img')";
            if (mysqli_query($conn, $q)) {
                echo "<script>alert('Add Slider Image!'); window.location.href='homepage_corosole.php';</script>";
            } else {
                echo "<script>alert('Failed to add product. Error: " . mysqli_error($conn) . "');</script>";
            }
        } else {
            echo "<script>alert('Failed to upload Slider Image.');</script>";
        }
    }
}
?>
...
<body>
    ...
    <div style="margin-bottom: 30px;">
        ...
        <p style="text-align: left; color: #c4c4c4; font-size: 1rem;">
            <a href="homepage_corosole.php" style="text-decoration: none; color: #c4c4c4;">Slider Images</a> 
            <span style="margin: 0 8px;">&gt;</span>
            <span style="color: rgb(244, 107, 44);">Add Slider Image</span>
        </p>
    </div>
    <form id="productForm" method="post" enctype="multipart/form-data">
    <label for="productName">Heading Text</label>
                <input type="text" id="productName" name="productName" placeholder="Enter heading text">
                
                <label for="description">Description Text</label>
                <textarea id="description" name="description" placeholder="Enter description text" rows="4"></textarea>
